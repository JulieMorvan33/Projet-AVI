...
class SpeedPredictions():
    ...
    def computeFLParam(self, fl):
        altitude = fl * 100 * FT2M  # altitude in meters
    ...
    def wind_correction(self, wind):
        # compute the ECON Mach wind correction, wind given in kt
        # wind negative if headwind, positive if tailwind
        # +1/2 point of Mach per 50kt headwind
        # -1/2 point of Mach per 50kt tailwind (given by Airbus Getting to Grips with Cost Index)
        self.econMach = min(self.MMO, round(self.econMach - wind/500*0.5, 3))

    def convertMachToTAS(self, temp):
        # temp given in Kelvin
        a = sqrt(GAMMA*R*temp) # sound speed in the air
